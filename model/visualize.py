...
def evaluate(model: Net, test_loader, device, params):
    model.to(device)
    model.eval()
    naive_language = pickle.load(open('./data/dataset/naive_language.pkl', 'rb'))
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            language, point_cloud, vision_global, vision_local, grasp_map, pos_index, neg_index, pos_neg_num, entry_paths = data
            if params['wol']:
                language = torch.from_numpy(naive_language).to(device).float()
                language = language.repeat(point_cloud.shape[0], 1)
            else:
            ...
            if params['maps_save']:
                ...
                if params['wol']:
                    pickle.dump(maps, open('./checkpoints/maps_{}wol_{}.pkl'.format(params['model_id'], i), 'wb'))
                else:
...
def visulaize_single(model: Net, dataset: LVDataset, device, params):
    model.to(device)
    model.eval()
    with torch.no_grad():
        ...
        entry_path = os.path.join(dataset_dir, object_id, '{}_{}.pkl'.format(object_id, config_id))
        with open(entry_path, 'rb') as f:
            ...
            grasp_likelihoods = config.grasp_likelihoods
            fragilities = config.fragilities
            parts_json = json.load(open(os.path.join(dataset_dir, object_id, 'parts.json'), 'r'))
            names = []
            for part in parts_json['parts'].values():
                names.append(part['name'])

            print(names, materials, frictions, densities, grasp_likelihoods, fragilities, sep='\n')
            
            

        data = dataset.get_entry(entry_path)
        language, point_cloud, vision_global, vision_local, grasp_map, pos_index, neg_index, pos_neg_num = data
        language = torch.from_numpy(language).to(device).float().unsqueeze(0)
        point_cloud = torch.from_numpy(point_cloud).to(device).float().unsqueeze(0)
        vision_global = torch.from_numpy(vision_global).to(device).float().unsqueeze(0)
        vision_local = torch.from_numpy(vision_local).to(device).float().unsqueeze(0)
        grasp_map = torch.from_numpy(grasp_map).to(device).float().unsqueeze(0)
        pos_index = torch.from_numpy(pos_index).to(device).long().unsqueeze(0)
        neg_index = torch.from_numpy(neg_index).to(device).long().unsqueeze(0)
        pos_neg_num = torch.from_numpy(pos_neg_num).to(device).long().unsqueeze(0)
            
        if params['global']:
            output_global = model(language, point_cloud, vision_global, vision_local)
            loss_global = model.get_loss(output_global, grasp_map)
            print(f'Loss Global {loss_global.item()}')
        else:
            output_global, output_local = model(language, point_cloud, vision_global, vision_local)
            loss_global, loss_local_pos, loss_local_neg = model.get_loss(output_global, output_local, grasp_map, pos_index, neg_index, pos_neg_num)
    np.set_printoptions(edgeitems=3, infstr='inf',
                    linewidth=75, nanstr='nan', precision=8,
                    suppress=False, threshold=np.inf, formatter=None)
    maps = {
        'point_cloud': point_cloud.squeeze().cpu().numpy(),
        'grasp_map': grasp_map.squeeze().cpu().numpy(),
        'prediction': output_global.squeeze().cpu().numpy(),
    }
    pickle.dump(maps, open('./checkpoints/maps.pkl', 'wb'))
    if params['gt']:
        print(grasp_map.squeeze().cpu().numpy())
        show_heatmap(point_cloud.squeeze().cpu().numpy(), grasp_map.squeeze().cpu().numpy())
    else:
        print(output_global.squeeze().cpu().numpy())
        show_heatmap(point_cloud.squeeze().cpu().numpy(), output_global.squeeze().cpu().numpy())
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--wol', default=False, action='store_true')
