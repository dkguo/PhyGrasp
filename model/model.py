...
class Net(nn.Module):
    ...
    def forward(self, language, point_cloud, vision_global, vision_local):
        ...
        global_local = torch.cat((global_feature, vision_local, point_cloud), dim=2) # (batch_size, 2048, 64+64+3)
    ...
    def get_pos(self, output_local, index1, point_cloud, kp2=1):
        '''
        output_local: (batch_size, 2048, 32)
        ...
        if kp2 == 1:
            ...
            index2 = torch.topk(score, kp2, dim=2).indices
            index_1 = index2.unsqueeze(3).expand(-1, -1, -1, point_cloud.shape[2])  # (batch_size, kp1, kp2, 3)
            pos1 = torch.gather(point_cloud.unsqueeze(1).expand(-1, index1.shape[1], -1, -1), 2, index_1)  # (batch_size, kp1, kp2, 3)
            index_2 = index1.unsqueeze(2).unsqueeze(3).expand(-1, -1, kp2, point_cloud.shape[2])  # (batch_size, kp1, kp2, 3)
            pos2 = torch.gather(point_cloud.unsqueeze(1).expand(-1, index1.shape[1], -1, -1), 2, index_2)  # (batch_size, kp1, kp2, 3)
            pos = torch.cat((pos1, pos2), dim=3)  # (batch_size, kp1, kp2, 6)
...
if __name__ == '__main__':
    ...
    net.to(device)
    net.train()
    language = torch.randn(2, 4096).to(device)
    point_cloud = torch.randn(2, 2048, 3).to(device)
    vision_global = torch.randn(2, 1024).to(device)
    vision_local = torch.randn(2, 2048, 64).to(device)
    output_global, output_local = net(language, point_cloud, vision_global, vision_local)
    # print(output_global.shape, output_global)
    # print(output_local.shape, output_local)
    pos_index = torch.randint(0, 2048, (2, 200, 2)).to(device)
    neg_index = torch.randint(0, 2048, (2, 200, 2)).to(device)
    loss_global, loss_local = net.get_loss(output_global, output_local, torch.randn(2, 2048).to(device), pos_index, neg_index)
    print(loss_global)
    print(loss_local)